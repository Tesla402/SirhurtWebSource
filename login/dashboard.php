...
require_once ('js-php/PartnerHandler.php');
...
else
{
    ...
    {
        $replace = file_get_contents("homepage.html");
        echo $replace;
        exit();
    }
...
if($stmt = $conn->prepare("SELECT authkey, hwid, email, username, pass, whitelistkey, ip, discord, date, expire, flagged, hwidlock, lastreset, flagreason, cookiesec, oldpwd, accesslevel, mac_hwid, mac_whitelistkey, mac_expire, cookiesec_expires, securitypin FROM UserDB WHERE username = ?")) {
   ...
   while ($stmt->fetch()) 
   {
        ...
        if ($usern == $username)
        {
            ...
            $replace = str_replace("<!--InsertAnnouncements-->", AnnouncementLog(), $replace);
            $replace = str_replace("<!--DeveloperAnnouncements-->", DeveloperLog(), $replace);
            ...
            /* Partner Ads -> 1-5-2021 */
            $generate_html = "";
            $html_for_ads = '<li class="list-group-item"><span class="text-muted">AD_DATE</span>
            <p>AD_MESSAGE</p></li>';
            
            $ads = Fetch_Todays_Ads();
            if (sizeof($ads) >= 1)
            {
                foreach($ads as $ad_item) 
                {
                    $new_ad = $html_for_ads;
                    $new_ad = str_replace("AD_DATE", strip_tags($ad_item[1] . " - " . $ad_item[0]), $new_ad);
                    $new_ad = str_replace("AD_MESSAGE", makeClickableLinks(strip_tags($ad_item[2])), $new_ad);
                    $generate_html = $generate_html . $new_ad;
                    
                    $generate_html = str_replace('\n', "<br>", $generate_html);
                    $generate_html = str_replace('\r', "", $generate_html);
                    $generate_html = str_replace("NO_PING", "", $generate_html);
                    $generate_html = str_replace("**", "", $generate_html);
                    $generate_html = str_replace("__", "", $generate_html);
                    $generate_html = str_replace("*", "", $generate_html);
                    $generate_html = str_replace("```css", "", $generate_html);
                    $generate_html = str_replace("```lua", "", $generate_html);
                    $generate_html = str_replace("```", "", $generate_html);
                    $generate_html = str_replace("~", "", $generate_html);
                    $generate_html = str_replace("[", "", $generate_html);
                    $generate_html = str_replace("]", "", $generate_html);
            ...
            $replace = str_replace("<!--PARTNER_ADS-->", $generate_html, $replace);
            /* End Partner Ads -> 1-5-2021 */
            
            /* System Announcements -> 1-6-2021 */
            $generate_html = "";
            $html_for_ads = '<li class="list-group-item"><span class="text-muted">AD_DATE</span>
            <p>AD_MESSAGE</p></li>';
            
            $ads_2 = Fetch_SirHurt_Announcements();
            if (sizeof($ads_2) >= 1)
            {
                foreach($ads_2 as $ad_item) 
                {
                    $new_ad = $html_for_ads;
                    $new_ad = str_replace("AD_DATE", strip_tags($ad_item[0] . " - " . $ad_item[1]), $new_ad);
                    $new_ad = str_replace("AD_MESSAGE", makeClickableLinks(strip_tags($ad_item[2])), $new_ad);
                    $generate_html = $generate_html . $new_ad;
            ...
            $replace = str_replace("<!--SYSTEM_ANNOUNCEMENTS-->", $generate_html, $replace);
            /* End Partner Ads -> 1-5-2021 */
