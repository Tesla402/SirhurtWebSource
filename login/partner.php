...
if($stmt = $conn->prepare("SELECT authkey, hwid, email, username, pass, whitelistkey, ip, discord, date, expire, flagged, hwidlock, lastreset, flagreason, cookiesec, oldpwd, accesslevel, mac_hwid, mac_whitelistkey, mac_expire, cookiesec_expires, securitypin FROM UserDB WHERE username = ?")) {
   ...
   while ($stmt->fetch()) 
   {
        ...
        if ($usern == $username)
        {
            ...
            $replace = file_get_contents("partner.html");
            ...
            $replace = str_replace("<!--InsertAnnouncements-->", HttpGet("https://sirhurt.net/login/js-php/announcementlog.php") , $replace);
            $replace = str_replace("<!--DeveloperAnnouncements-->", HttpGet("https://sirhurt.net/login/js-php/developerlog.php") , $replace);
